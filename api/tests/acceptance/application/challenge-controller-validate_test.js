...
describe('Acceptance | API | ChallengeController', function() {
  ...
  describe('PUT /api/challenges/:challenge_id/validate', function() {
    ...
    it('should be able to transform all related answer, but not unrelated answer(s)', () => {
      return server.inject(options).then(() => {
        return knex.select('*').from('answers').then((answers) => {
    ...
    it('should be able to change "ok", "ko", "partially", "unimplemented"', () => {
      return server.inject(options).then((response) => {
    ...
    it('should NOT be able to change "timedout", "aband"', () => {
      return server.inject(options).then((response) => {
        const payload = response.payload;
        const result = JSON.parse(payload);
